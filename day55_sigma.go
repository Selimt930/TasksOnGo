//https://www.hackerrank.com/challenges/detect-html-links/problem?h_r=internal-search
package main
...
var reg = regexp.MustCompile(`(?i)< *a *href *= *"`)
var regClose = regexp.MustCompile(`(?i)< */ *a *>`)
...
func main() {
    ...
    all_groups := reg.FindAllStringSubmatchIndex(all_doc, -1)
    ...
    for i := range all_groups {
        ...
        content_inds := regClose.FindStringIndex(all_doc[atag_angclose:])
